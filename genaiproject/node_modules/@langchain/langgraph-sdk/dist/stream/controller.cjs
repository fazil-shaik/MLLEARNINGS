...
var StreamController = class {
	...
	*/
	#missingThreadIds = /* @__PURE__ */ new Set();
	/**
	...
	*/
	constructor(options) {
		...
		this.#submitter = new require_submit_coordinator.SubmitCoordinator({
			...
			isSelfCreatedThreadId: (threadId) => this.#selfCreatedThreadIds.has(threadId) || this.#missingThreadIds.has(threadId),
			...
			*/
			if (Array.isArray(state?.tasks)) {
				...
				for (const task of state.tasks) {
					...
					const checkpointNs = task?.checkpoint?.checkpoint_ns;
					const namespace = typeof checkpointNs === "string" && checkpointNs.length > 0 ? checkpointNs.split("|").filter((segment) => segment.length > 0) : [...ROOT_NAMESPACE];
					for (const interrupt of task.interrupts) {
						...
						activeInterrupts.push(require_interrupts.normalizeInterruptForClient({
							id,
							value: typed?.value,
							namespace: Array.isArray(typed?.namespace) ? [...typed.namespace] : namespace
						}));
					}
				}
				this.rootStore.setState((s) => ({
	...
	*/
	async #trySeedProjectionFromHistory(params) {
		const threadId = this.#currentThreadId;
		if (this.#disposed || threadId == null || params.namespace.length === 0 || !this.#rootPumpDeferred || this.#selfCreatedThreadIds.has(threadId) || this.#missingThreadIds.has(threadId)) return false;
	...
	*/
	async respond(response, options) {
		...
		const resolved = options?.interruptId != null ? {
			...
			namespace: this.#resolveNamespaceForInterruptId(options.interruptId, options.namespace)
 ...
 /**
				...
				*/
				while (!this.#disposed && pumpGeneration === this.#rootPumpGeneration) {
					for await (const event of subscription) {
						if (this.#disposed || pumpGeneration !== this.#rootPumpGeneration) break;
						/**
	...
	*/
	#recordInterrupt(event) {
		...
		const namespace = Array.isArray(event.params.namespace) ? [...event.params.namespace] : [...ROOT_NAMESPACE];
	...
	*/
	#resolveNamespaceForInterruptId(interruptId, explicitNamespace) {
		if (explicitNamespace != null) return [...explicitNamespace];
		const fromThread = this.#thread?.interrupts.find((entry) => entry.interruptId === interruptId)?.namespace;
		if (fromThread != null) return [...fromThread];
		const fromRoot = this.rootStore.getSnapshot().interrupts.find((entry) => entry.id === interruptId)?.namespace;
...
function extractAndCoerceMessagesWithFallback(values, messagesKey) {
	...
	if (!Array.isArray(raw) && messagesKey !== "messages") raw = values[require_constants.DEFAULT_MESSAGES_KEY];
